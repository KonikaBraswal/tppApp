...
class SanboxApiClient {
  ...
  async fetchAccounts(apiAccessToken: string): Promise<any> {
    try {
      ...
      const accountIds = acDetails.Account.map((account: { AccountId: any; }) => account.AccountId);
  ...
  async domesticPayments(apiAccess: string): Promise<any> {
    try {
      ...
      return this.getPaymentSatus(apiAccess,paymentResponse.data.Data.DomesticPaymentId);
    } catch (error) {
      throw new Error(`Failed to fetch data for accounts: ${error}`);
    }
  }
  async getPaymentSatus(apiAccessToken:string,domesticPaymentsId:string):Promise<any>{
    try {
      const headers = {
        "Authorization": 'Bearer ' + apiAccessToken,
        "x-fapi-financial-id": sandboxConfig.financialId      

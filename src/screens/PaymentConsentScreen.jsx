...
const PaymentConsentScreen = ({ route }) => {
  ...
  const handleConfirmButtonClick = async () => {
      try {
        const consentData = await apiClient.retrieveAccessToken_pisp(
          'payments',
          DebtorAccount,
        );
        ...
        if (way == 'web') {
          ...
          const consentUrl = await apiClient.manualUserConsent_pisp(
            consentData,
          );
  ...
  const handleSubmit = async () => {
    try {
      const data = await apiClient.exchangeAccessToken_pisp(inputValue);
